...
import QuestionItemAdm from '../../components/QuestionItemAdm/QuestionItemAdm';
import QuestionListAdm from '../../components/QuestionListAdm/QuestionListAdm';
import QuestionComponent from '../../components/ReponseQuestion/ReponseQuestion';
...
const BachelierAdmin = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.REACT_APP_API_URL}/all/questions/`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${token}`, // Ajouter le token dans les en-têtes
        'Content-Type': 'application/json' // Assurez-vous que le type de contenu est correct
  ...
  const handleReply = (id, event) => {
    ...
    setShowReponse(true)
      ...
      return (
        ...
        {showReponse && <QuestionComponent currentQuestion={selectedQuestion} onReturn={handleReturn}/>}
        <QuestionListAdm
 questions={questions}
 onSelect={handleSelectQuestion}
 onDelete={handleDelete}
